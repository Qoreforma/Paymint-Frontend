...
import VerifyNewPhoneNoForm from "./VerifyNewPhoneNoForm";
        ...
        <AlertDialog open={openDialog} onOpenChange={setOpenDialog}>
            <AlertDialogTrigger asChild>
                <button onClick={() => setOpenDialog(prev => !prev)} className="cursor-pointer flex items-center gap-[9px] py-3.5 pr-3.5 text-[#344054] border-b-[0.1px] border-[#667085]/30 transition">
                    ...
                    <span className="text-[13px]">{user?.phoneVerifiedAt ? "View phone number" : "Update phone number"}</span>
            ...
            <AlertDialogContent className="bg-white border-0 rounded-lg w-[95vw] !max-w-[430px] h-auto !md:min-h-[476px] !p-0 md:!p-10">
                <div className="w-full flex items-center h-[61px] md:hidden border-b border-[#E9E9E97D] px-4">
                    ...
                    <p className="text-[#1C1C1C] text-lg w-full text-center mr-6">{user?.phoneVerifiedAt ? "Phone number" : "Update Phone number"}</p>
                </div>
                <div className="w-[90%] max-w-[350px] mx-auto max-md:pb-5">
                    <AlertDialogHeader className="gap-0 max-md:text-left">
                        <AlertDialogTitle className="flex flex-col justify-center">
                            ...
                            <span className="text-[var(--aqua)] font-medium md:font-bold text-[22px] md:text-[28px]">{user?.phoneVerifiedAt ? "Phone number" : "Update Phone number"}</span>
                        </AlertDialogTitle>
                        <AlertDialogDescription className="text-[var(--ink)]">
                            {user?.phoneVerifiedAt ? "Phone number linked to your account" : "Please enter a valid phone number to update."}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <div className="mt-5">
                        <VerifyNewPhoneNoForm disableBackButton={disableBackButton} closeModal={closeModal} />
                    </div>
                </div>
            </AlertDialogContent>
        </AlertDialog>
