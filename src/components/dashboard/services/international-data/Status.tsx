...
const Status = () => {
    const {txnResult, phone, reset, product, country} = usePurchaseIntDataStore();
    ...
    <div className="grid place-items-center min-h-full w-full">
        <section className="w-full max-w-[325px] flex flex-col items-center">
            ...
            <p className="md:text-xl md:font-medium text-center mt-3 mb-12 md:my-10 max-md:text-[#717171]">
                Your <span className="font-medium">{product?.name}</span> purchase for <span className="font-medium">{country?.currencySymbol}{product?.amount}</span> on <span className="font-medium">{phone}</span> {txnResult ? "is successful" : "failed"}.
            </p>
