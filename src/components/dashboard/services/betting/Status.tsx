...
const Status = () => {
    const {txnResult, provider, user, amount, reset}  = useBettingStore();
    ...
    <div className="grid place-items-center min-h-full w-full">
        <section className="w-full max-w-[325px] flex flex-col items-center">
            ...
            <p className="md:text-xl text-center mt-3 mb-12 md:my-10 max-md:text-[#717171]">
                Your <span className="font-medium">{provider}</span> account ({user}) funding with <span className="font-medium">{formatAmount(parseInt(amount))}</span> {txnResult ? "is successful" : "failed"}.
            </p>
