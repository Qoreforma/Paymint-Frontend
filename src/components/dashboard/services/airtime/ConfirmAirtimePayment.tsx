...
import { buyAirtime } from "@/lib/api/dashboard-apis/servicesApis";
...
const ConfirmAirtimePayment = () => {
  ...
  const {update, step, phone, amount, provider, useCashback, cashbackRule} = useServiceFlowStore();
  ...
  const handleSubmit = (e?: FormEvent) => {
      ...
      mutate({amount, phone, pin: paymenTPin, provider: provider, useCashback: useCashback })
  }
  ...
  return (
    <div className="min-h-full flex md:items-center justify-center relative">
        <section className="w-full max-w-[358px] mx-auto flex flex-col max-md:justify-center max-md:text-center">
            ...
            <p className="text-[#101828] md:text-xl mt-5 md:mt-8 mb-4">You are about to purchase <span className="font-medium">{formatAmount(parseInt(amount))}</span> <span className="font-medium">Airtime</span> on <span className="font-medium">{phone}</span>.</p>
