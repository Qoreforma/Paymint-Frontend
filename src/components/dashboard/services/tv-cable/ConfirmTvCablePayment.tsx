...
import { subscribeToTv } from "@/lib/api/dashboard-apis/servicesApis";
import { AxiosError } from "axios";
import { toast } from "sonner";
import { TvSubscriptionResponse } from "@/stores/useTVCableStore";

const ConfirmTvCablePayment = () => {
  const [paymenTPin, SetPaymenTPin] = useState<string>("");
  const [showPinForm, setShowPinForm] = useState(false);

  const isMobile = useIsMobile();
  const { update, step, provider, package: selectedPackage, smartCardNo} = useTVCableStore();

  const {mutate, isPending} = useMutation({
      mutationFn: subscribeToTv,
      onSuccess: (data) => {
        update({step: step+1, txnResult: data as TvSubscriptionResponse});
      },
  ...
  const handleSubmit = (e?: FormEvent) => {
      ...
      mutate({provider: provider.code, number: smartCardNo, productId: String(selectedPackage.id), pin: paymenTPin, type: "renew" })
  }
  ...
  return (
    <div className="min-h-full flex md:items-center justify-center relative">
        <section className="w-full max-w-[358px] mx-auto flex flex-col max-md:justify-center max-md:text-center">
            ...
            <p className="text-[#101828] md:text-xl my-5 md:my-8">You are about to purchase <span className="font-medium">{selectedPackage?.name}</span> to <span className="font-medium"> ({smartCardNo})</span>.</p>
...
export default ConfirmTvCablePayment;