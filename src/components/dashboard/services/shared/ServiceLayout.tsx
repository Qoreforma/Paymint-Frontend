...
import { Loader2, ArrowLeft, Plus, Eye } from 'lucide-react';
...
const ServiceLayout = ({
  ...
  const { data: historyData, isLoading: fetchingHistory } = useQuery({
    queryKey: ["txn-history", serviceType],
    queryFn: () => getTxnHistory({ per_page: 3, type: serviceType }),
  });
        ...
        <div className="flex-1 flex items-center justify-center max-w-md mx-auto w-full">
          <div className="flex items-center w-full justify-between relative">
            ...
            {Array.from({ length: totalSteps }).map((_, i) => {
              ...
              return (
                <div key={stepNum} className="flex items-center gap-2 bg-[#F9FAFB] px-2">
                  <div className={`size-6 rounded-full flex items-center justify-center text-xs font-semibold
                    ${isActive || isPast ? 'bg-blue-600 text-white' : 'bg-slate-200 text-slate-500'}`}
                  >
                    {stepNum}
                  </div>
                  <span className={`text-sm font-medium ${isActive || isPast ? 'text-blue-600' : 'text-slate-400'}`}>
        ...
        <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-6">
          ...
          <div className="bg-gradient-to-br from-[#0B45C8] to-[#082E85] rounded-2xl p-6 text-white relative overflow-hidden shadow-md">
            <div className="absolute right-[-20px] top-[-20px] opacity-10 pointer-events-none">
              ...
              <div className="size-40 rounded-full border-[20px] border-white"></div>
            </div>
          ...
          {/* Transaction Summary (Only show if there's data) */}
          {Object.keys(summaryData).length > 0 && (
            <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
              <h3 className="text-slate-800 font-display font-semibold mb-6">Transaction Summary</h3>
              <div className="flex flex-col gap-4">
                {Object.entries(summaryData).map(([key, value]) => (
                  <div key={key} className={`flex items-center justify-between ${key.toLowerCase() === 'total' ? 'pt-4 border-t border-slate-100 font-semibold' : 'text-sm'}`}>
                    <span className="text-slate-500 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                    <span className={key.toLowerCase() === 'total' ? 'text-blue-600' : 'text-slate-800'}>{value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
               <h3 className="text-slate-800 font-display font-semibold">Recent {serviceTitle} Purchases</h3>
            ...
            <div className="flex flex-col gap-5">
              {fetchingHistory ? (
                 ...
                 <p className="text-slate-500 text-sm text-center py-4">No recent transactions found.</p>
              ) : (
                recentTxns.map((txn) => (
                  <div key={txn.id} className="flex items-center justify-between group cursor-pointer">
                     <div className="flex items-center gap-3">
                         <div className="size-10 rounded-full bg-slate-50 border border-slate-100 p-2 flex items-center justify-center overflow-hidden">
                             {txn.metadata?.logo ? (
                                <img src={txn.metadata.logo} alt="" className="w-full h-full object-contain" />
                             ) : (
                                <div className="size-full bg-blue-100 rounded-full" />
                             )}
                         </div>
                         <div>
                            <p className="text-sm font-medium text-slate-800">{txn.metadata?.phone || txn.metadata?.smartCardNumber || txn.metadata?.meterNumber || 'Transaction'}</p>
            ...
            <Link to={`/dashboard/history?type=${serviceType}`} className="mt-6 block w-full py-3 bg-slate-50 hover:bg-slate-100 text-center text-blue-600 text-sm font-medium rounded-xl transition-colors">
