...
const Status = () => {
    const {txnResult, quantity, meterNumber, reset} = useElectricityBillsStore();
  ...
  return (
    <div className="grid place-items-center min-h-full w-full">
        <section className="w-full max-w-[325px] flex flex-col items-center">
            ...
            Your payment of <span className="font-medium">{formatAmount(parseInt(quantity))}</span> on <span className="font-medium">{meterNumber}</span> {txnResult ? "is successful" : "failed"}.
            </p>
